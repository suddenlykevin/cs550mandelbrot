...
# MANDELBROT 1: LAME IMPALA (based on https://wallpapercave.com/w/wp1901421)
		...
		# if Z does escape, when the escape iteration is even, then space is black, else space is a gradient from light pink to light purple
			...
			if t%2==1 and t>tmax/10:
				R,G,B=int(colorsys.hsv_to_rgb(0.9-(t-tmax/10)/(tmax/4),1,1)[0]*255), int(colorsys.hsv_to_rgb(0.9-(t-tmax/10)/(tmax/4),1,1)[1]*255), int(colorsys.hsv_to_rgb(0.9-(t-tmax/10)/(tmax/4),1,1)[2]*255)
...
for x in range(imgx): 
	for y in range(imgy):
		...
		zi,zx = (y*(yb3-ya3)/(imgy-1)+ya3),(x*(xb3-xa3)/(imgx-1)+xa3) 
		for t in range(tmax):
			...
			if absz>=2.0: 
				break
			zx,zi=float((zx*zx-zi*zi)+cx),float((zi*zx+zx*zi)+cy)
		# cycles through HSV starting from yellow down to green
